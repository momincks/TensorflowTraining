...
class Loader:
    ...
    # use dir_map to map directories to classes
    # map to label "frame" to random crop it and train it as background

    def __init__(self, size_h, size_w, batch, val_split, shuffle, classes, dir_map,
        ...
        self.max_intensity = {k:v*augment_multiple for k,v in self.min_intensity}
        self.off_value = 1. - self.on_value/(num_class-1)
    ...
    # augment for batched image
        ...
        # random cutout
        if tf.random.uniform([]) < 1:
            size = tf.random.uniform([2], self.intensity["cutout_min"], self.intensity["cutout_max"], dtype=tf.int32)
            img = tfa.image.random_cutout(image, size)
        # resize and padding
    ...
    def more_intense(self):
        for augment in self.intensity.keys():
            ...
            self.intensity[augment] = max(self.intensity[augment], self.max_intensity[augment])
...
class TFRecordLoader(Loader):
    ...
    def __init__(self, 
        ...
        super(TFRecordLoader, self).__init__(size_h, size_w, batch, val_split, shuffle, classes, dir_map, augment_multiple, augment_step, min_intensity)
        self.train_dir, self.train_cls, self.val_dir, self.val_cls = [], [], [], []
        self.train, self.val = {}, {}
        self.train_info, self.val_info = dict(zip(self.classes,[0]*self.num_class)), dict(zip(self.classes,[0]*self.num_class))
...
class ImageLoader(Loader):
    ...
    def __init__(self, 
        ...
        super(ImageLoader, self).__init__(size_h, size_w, batch, val_split, shuffle, classes, dir_map, augment_multiple, augment_step, min_intensity)
        self.train_dir, self.train_cls, self.val_dir, self.val_cls = [], [], [], []
        self.train, self.val = {}, {}
        self.train_info, self.val_info = dict(zip(self.classes,[0]*self.num_class)), dict(zip(self.classes,[0]*self.num_class))
        for map_dir in dir_map.keys():
    ...
    def train_read(self, img, label):
        ...
        if min(tf.shape(img)[:2]) < self.size_w//8 and label != "background":
            ...
            on_value, off_value = self.on_value-0.3, self.off_value-0.3/(self.num_class-1)
            label = tf.argmax(label == self.classes)
            label = tf.one_hot(label, len(self.classes), on_value, off_value)
        elif min(tf.shape(img)[:2]) < self.size_w//4 and label != "background":
            img = tf.image.resize(img,[self.size_h,self.size_w],"bicubic")
            on_value, off_value = self.on_value-0.15, self.off_value-0.15/(self.num_class-1)
            label = tf.argmax(label == self.classes)
            label = tf.one_hot(label, len(self.classes), on_value, off_value)   
        # random crop for label "frame"              
        elif tf.math.equal(label,"frame"):
            size_h = min(tf.random.uniform([],self.size_h//32,self.size_h*2,dtype=tf.int32).numpy(),tf.shape(img)[0]-1)
            size_w = min(int(tf.random.uniform([],0.5,2.0).numpy()*size_h),tf.shape(img)[1]-1)
            ...
            img = img[anchor_y:anchor_y+size_h,anchor_x:anchor_x+size_w,:]
            img = tf.image.resize(img,[self.size_h,self.size_w],"bicubic")
            label = tf.argmax(self.wrong_label == self.classes)
            label = tf.one_hot(label, len(self.classes), self.on_value, self.off_value)
